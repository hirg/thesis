...
#include "EventCutMaker/EventCutMaker.h"
...
void StFlowAnalysisMaker::PsiShift() {
  ...
  for (int k = 0; k < Flow::nSels; k++) {
    for (int j = 0; j < Flow::nHars; j++) {
  	    if(mFillPsiShift) {
	        int i = Flow::nSels*k;
			FillPsiShift(mPsi[k][j],k,j,0);
			FillPsiShift(mPsiSub[i][j],k,j,1);
			FillPsiShift(mPsiSub[i+1][j],k,j,2);
  		} else if(mDoPsiShift) {
	        int i = Flow::nSels*k;
			mPsi[k][j] += DoPsiShift(mPsi[k][j],k,j,0);
			mPsiSub[i][j] += DoPsiShift(mPsiSub[i][j],k,j,1);
			mPsiSub[i+1][j] += DoPsiShift(mPsiSub[i+1][j],k,j,2);
  		}
    } // nHars
  } // nSels
} 
...
void StFlowAnalysisMaker::FillPsiShift(Float_t rawPsi, Int_t sel, Int_t harm, Int_t sub) {
  // Fill Psi-shift coefficients 
	...
	{
	  mPsiShiftCos[sel][harm][sub]->Fill(i, cos(harm*i*rawPsi));
	  mPsiShiftSin[sel][harm][sub]->Fill(i, sin(harm*i*rawPsi));
	}
...
Float_t StFlowAnalysisMaker::DoPsiShift(Float_t rawPsi, Int_t sel, Int_t harm, Int_t sub) {
  // Calculate the psi-shift
	...
	{
        temp = 0;
		temp += mPsiShiftCos[sel][harm][sub]->GetBinContent(i)*sin(harm*i*rawPsi);
		temp -= mPsiShiftSin[sel][harm][sub]->GetBinContent(i)*cos(harm*i*rawPsi);
		temp /= i;
        deltaPsi += temp;
	}
...
void StFlowAnalysisMaker::SetEventCut(EventCutMaker* eventCut) {
